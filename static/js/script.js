...
class PomodoroApp {
  ...
  async loadData() {
    try {
      /* Режим офлайн, если открыт по file:// */
      const IS_OFFLINE =
        typeof location !== "undefined" && location.protocol === "file:";

      console.log("Режим работы:", IS_OFFLINE ? "Офлайн" : "Онлайн");
      ...
      if (IS_OFFLINE) {
        console.log("Используем офлайн данные");
      ...
      console.log("Места загружены:", this.places.length);

      // Загружаем стандартные интервалы Pomodoro (5, 15, 25, 45 минут)
      const presetsResponse = await fetch("/api/timer/presets");
      if (!presetsResponse.ok) {
        throw new Error(`HTTP error! status: ${presetsResponse.status}`);
      }
      this.timerPresets = await presetsResponse.json();
      console.log("Пресеты загружены:", this.timerPresets.length);
    } catch (error) {
      console.error("Ошибка загрузки данных:", error);

      // Более подробная диагностика ошибки
      let errorMessage = "Ошибка загрузки данных";

      if (error.name === "TypeError" && error.message.includes("fetch")) {
        errorMessage =
          "Ошибка сети при загрузке данных. Проверьте подключение к интернету.";
      } else if (error.message.includes("HTTP error")) {
        errorMessage = `Ошибка сервера при загрузке данных: ${error.message}`;
      } else if (error.message.includes("Failed to fetch")) {
        errorMessage = "Не удается подключиться к серверу для загрузки данных.";
      }

      console.warn(errorMessage);

      // В случае ошибки используем офлайн данные как fallback
      console.log("Используем офлайн данные как fallback");
      this.places = [
        {
          id: "work",
          name: "Работа",
   ...
   */
  renderPlaces() {
    console.log("Рендерим места:", this.places);

    const placesGrid = document.getElementById("places-grid");
    const placesButtons = document.getElementById("places-buttons");

    if (!placesGrid || !placesButtons) {
      console.error("Не найдены элементы для рендеринга мест");
      return;
    }
    ...
    console.log(
      "Карточки мест созданы:",
      placesGrid.innerHTML.length,
      "символов"
    );
    ...
    console.log(
      "Кнопки мест созданы:",
      placesButtons.innerHTML.length,
      "символов"
    );
    ...
    placesGrid.addEventListener("click", (e) => {
      ...
      if (placeCard) {
        ...
        console.log("Выбрано место:", placeId);
  ...
  async startTimer(duration) {
    console.log("Попытка запуска таймера:", duration, "минут");
    ...
    if (!this.currentPlace) {
      ...
      alert("Сначала выберите место для работы");
      return;
    }
    ...
    if (IS_OFFLINE) {
      console.log("Запускаем таймер в офлайн режиме");
      // Генерируем локальный timer_id и запускаем локальный таймер
    ...
    try {
      console.log("Отправляем запрос на сервер для запуска таймера...");
      ...
      console.log("Результат запуска таймера:", result);
      ...
      if (result.status === "started") {
        ...
        throw new Error(`Неожиданный статус ответа: ${result.status}`);
      }
    } catch (error) {
      console.error("Ошибка запуска таймера:", error);

      // Более подробная диагностика ошибки
      let errorMessage = "Не удалось запустить таймер";
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        ...
        errorMessage =
          "Не удается подключиться к серверу. Попробуйте обновить страницу.";
      }

      alert(errorMessage);

      // В случае ошибки сети, предлагаем запустить в офлайн режиме
      if (error.name === "TypeError" || error.message.includes("fetch")) {
        const useOffline = confirm("Хотите запустить таймер в офлайн режиме?");
        if (useOffline) {
          console.log("Запускаем таймер в офлайн режиме после ошибки сети");
          this.currentTimer = `timer_${Date.now()}`;
          this.startTimerDisplay(duration);
          this.animateCharacter(true);
          this.enterFocusMode();
  ...
  stopTimer() {
    ...
    if (typeof location !== "undefined" && location.protocol === "file:") {
      ...
      if (this.currentTimer) {
        // В офлайн-режиме нет API, поэтому просто удаляем локальный timer_id
        this.currentTimer = null;
      }
    } else {
      if (this.currentTimer) {
  ...
  applyCharacterImageForPlace() {
    ...
    // Устанавливаем напрямую относительный путь — работает и для http(s), и для file://
    img.src = nextSrc;
  }
