...
class PomodoroApp {
  ...
  async loadData() {
    ...
    try {
      const [placesResponse, presetsResponse] = await Promise.all([
        fetch("/api/places"),
        fetch("/api/timer/presets"),
      ]);

      this.places = await placesResponse.json();
      this.timerPresets = await presetsResponse.json();
    } catch (error) {
      // Fallback на офлайн данные
      this.places = [
        {
          id: "work",
          name: "Работа",
  ...
  async startTimer(duration) {
    ...
    try {
      const response = await fetch("/api/timer/start", {
        ...
        body: JSON.stringify({ duration, place: this.currentPlace.id }),
      });
      ...
      const useOffline = confirm("Запустить в офлайн режиме?");
      if (useOffline) {
        this.currentTimer = `timer_${Date.now()}`;
        this.startTimerDisplay(duration);
        this.enterFocusMode();
  ...
  stopTimer() {
    ...
    if (this.currentTimer && location.protocol !== "file:") {
